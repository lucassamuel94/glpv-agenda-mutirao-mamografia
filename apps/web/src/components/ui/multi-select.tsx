...
export function MultiSelect({
  ...
  single?: boolean;
  /**
   * Quando `true` (default), o popover bloqueia interação fora. Passe `false`
   * quando usar dentro de outro modal (Dialog) — caso contrário o outside-click
   * é interceptado pelo modal externo e o popover não fecha.
   */
  modal?: boolean;
}) {
      ...
      {[...selectedValues]
        ...
        .map((value) => (
          <Badge
            ...
            data-selected-item
            className={cn(
              "group flex shrink-0 items-center gap-1 border-border text-foreground",
    ...
    <>
      ...
      <PopoverContent className="min-w-[var(--radix-popover-trigger-width)] p-0">
        <Command {...props}>
          {canSearch ? (
            <CommandInput
              className="border-none outline-none focus:ring-0_ focus:border-0 px-2 py-1 !bg-transparent shadow-transparent"
              placeholder={
...
export function MultiSelectItem({
  ...
  value: string;
  /** Optional: string used for filter/search (defaults to value) */
  searchValue?: string;
